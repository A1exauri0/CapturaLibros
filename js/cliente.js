...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const capaRecorteInteractiva = document.getElementById(
    "capa-recorte-interactiva",
  );
  ...
  const btnMenuRenombrar = null; // Removido del DOM
  const btnMenuRecortar = document.getElementById("btn-menu-recortar");
  const btnMenuCancelarRecorte = document.getElementById(
    "btn-menu-cancelar-recorte",
  );
  const btnMenuCortar = document.getElementById("btn-menu-cortar");
  const btnMenuCancelarCrear = document.getElementById(
    "btn-menu-cancelar-crear",
  );

  const visorControlesBarra = document.getElementById("visor-controles-barra");
  const contenedorRejillaMiniaturas = document.getElementById(
    "contenedor-rejilla-miniaturas",
  );
  const rejillaMiniaturasPdf = document.getElementById(
    "rejilla-miniaturas-pdf",
  );
  ...
  // Elementos del Panel de Control / Ventana Modal de Creación
  const modalCrearPdf = document.getElementById("modal-crear-pdf");
  const inputNuevoNombre = document.getElementById("modal-input-nombre");
  const textoReglaEjemplo = document.getElementById(
    "modal-texto-regla-ejemplo",
  );
  ...
  if (formularioLogin) {
    formularioLogin.addEventListener("submit", async (e) => {
      ...
      if (
        window.apiProyecto &&
        typeof window.apiProyecto.iniciarSesion === "function"
      ) {
        try {
          errorLogin.textContent = "";
          const botonSubmit = formularioLogin.querySelector(
            'button[type="submit"]',
          );
          botonSubmit.disabled = true;
          const textoOriginalBtn = botonSubmit.innerHTML;
          botonSubmit.innerHTML = `<div class="circulo-carga" style="width: 14px; height: 14px; border-width: 2px; display: inline-block; margin-right: 6px; vertical-align: middle;"></div><span>Cargando...</span>`;
          ...
          errorLogin.textContent =
            "Error de comunicación local con el servidor.";
          formularioLogin.querySelector('button[type="submit"]').disabled =
            false;
        }
      } else {
  ...
  async function cargarListaPDFs() {
    if (
      window.apiProyecto &&
      typeof window.apiProyecto.obtenerPDFs === "function"
    ) {
      try {
        ...
        if (cargadorVisor) {
          ...
          if (textoCarga)
            textoCarga.textContent = "Buscando libros en la red...";
        }
        ...
        const pdfNoProcesado = archivosPdfs.find(
    ...
    if (visorPlaceholder) {
      ...
      if (placeholderTitulo && placeholderTexto) {
        ...
        placeholderTexto.textContent =
          "No quedan más libros PDF pendientes de procesar en la red local.";
      }
    }
    if (visorActivo) {
  ...
  function seleccionarArchivo(archivo) {
    ...
    if (
      window.apiProyecto &&
      typeof window.apiProyecto.obtenerRutaArchivo === "function"
    ) {
      const urlSegura = window.apiProyecto.obtenerRutaArchivo(
        archivo.rutaLocal,
      );
  ...
  function actualizarMenuLateral() {
    if (!archivoSeleccionado) {
      ...
      if (btnMenuCancelarCrear) btnMenuCancelarCrear.disabled = true;

      if (btnMenuRenombrar)
        btnMenuRenombrar.classList.remove("activo", "ejecutar");
      if (btnMenuRecortar)
        btnMenuRecortar.classList.remove("activo", "ejecutar", "oculto");
      if (btnMenuCortar)
        btnMenuCortar.classList.remove("activo", "ejecutar", "oculto");
      if (btnMenuCancelarCrear)
        btnMenuCancelarCrear.classList.remove("activo", "ejecutar", "oculto");

      if (btnMenuRecortar)
        btnMenuRecortar.querySelector("span").textContent = "Recortar";
      if (btnMenuCortar)
        btnMenuCortar.querySelector("span").textContent = "Crear PDF";
      return;
    }
    ...
    if (btnMenuCancelarCrear) btnMenuCancelarCrear.disabled = false;

    // 1. Botón Renombrar (Acción directa siempre - Removido del DOM)
    if (btnMenuRenombrar)
      btnMenuRenombrar.classList.remove("activo", "ejecutar");

    // 2. Botón Recortar
    if (btnMenuRecortar) {
      // Ocultar Recortar si se está en modo de creación de PDF
      if (modoActual === "crear") {
        btnMenuRecortar.classList.add("oculto");
      } else {
        btnMenuRecortar.classList.remove("oculto");
      }
    ...
    // Mostrar/ocultar botón Cancelar Recorte
    if (btnMenuCancelarRecorte) {
      if (modoActual === "recortar" && herramientaRecorteActiva) {
        btnMenuCancelarRecorte.classList.remove("oculto");
      } else {
        btnMenuCancelarRecorte.classList.add("oculto");
      }
    }

    // Mostrar/ocultar botón Cancelar Creación
    if (btnMenuCancelarCrear) {
      if (modoActual === "crear") {
        btnMenuCancelarCrear.classList.remove("oculto");
      } else {
        btnMenuCancelarCrear.classList.add("oculto");
      }
    }

    // 3. Botón Cortar (Crear PDF)
    if (btnMenuCortar) {
      // Ocultar Crear PDF si la herramienta de recortar página está activa
      if (modoActual === "recortar" && herramientaRecorteActiva) {
        btnMenuCortar.classList.add("oculto");
      } else {
        btnMenuCortar.classList.remove("oculto");
      }
      ...
      if (modoActual === "crear") {
        ...
        if (totalSeleccionadas > 0) {
          ...
          btnMenuCortar.querySelector("span").textContent =
            `Crear PDF (${totalSeleccionadas})`;
        } else {
    ...
    btnMenuCortar.addEventListener("click", () => {
      ...
      if (modoActual !== "crear") {
        ...
        if (paginasSeleccionadas.size > 0) {
          ...
          alert(
            "Por favor, selecciona al menos una hoja de la cuadrícula de miniaturas para crear el nuevo PDF.",
          );
        }
      }
    });
  ...
  function alternarHerramientaRecorte(forzarEstado) {
    ...
    if (capaRecorteInteractiva) {
      capaRecorteInteractiva.classList.toggle(
        "activa",
        herramientaRecorteActiva,
      );
    }
  ...
  async function cargarMiniaturasRejilla() {
    if (!pdfDocumento || miniaturasCargadasDocId === archivoSeleccionado.id)
      return;
  ...
  async function renderizarMiniatura(numPag) {
    try {
      ...
      // Escala aumentada a 1.2 para renderizar las hojas totalmente nítidas y de excelente tamaño
  ...
  function inicializarGuiasRecorte() {
    ...
    elementoLineaSuperior.className =
      "linea-recorte linea-recorte-horizontal linea-superior";

    elementoLineaInferior = document.createElement("div");
    elementoLineaInferior.className =
      "linea-recorte linea-recorte-horizontal linea-inferior";

    elementoLineaIzquierda = document.createElement("div");
    elementoLineaIzquierda.className =
      "linea-recorte linea-recorte-vertical linea-izquierda";

    elementoLineaDerecha = document.createElement("div");
    elementoLineaDerecha.className =
      "linea-recorte linea-recorte-vertical linea-derecha";
  ...
  function actualizarPosicionesGuias() {
    ...
    if (elementoLineaSuperior)
      elementoLineaSuperior.style.top = `${ySuperior}px`;
    if (elementoLineaInferior)
      elementoLineaInferior.style.top = `${yInferior}px`;
    if (elementoLineaIzquierda)
      elementoLineaIzquierda.style.left = `${xIzquierda}px`;
  ...
  window.addEventListener("mousemove", (e) => {
    ...
    const actualX = Math.max(
      0,
      Math.min(e.clientX - rectCapa.left, rectCapa.width),
    );
    const actualY = Math.max(
      0,
      Math.min(e.clientY - rectCapa.top, rectCapa.height),
    );
  ...
  async function ejecutarRecorteGeometrico() {
    ...
    if (!herramientaRecorteActiva || !cajaRecorteElemento) {
      alert(
        "Activa la herramienta de recorte y ajusta las guías antes de recortar.",
      );
      return;
    }
    ...
    const confirmar = confirm(
      `¿Estás seguro de que deseas recortar los márgenes de la página ${paginaActiva} y guardar el resultado directamente en el PDF original?\n\nEsta acción modificará físicamente el archivo.`,
    );
    if (!confirmar) return;

    if (
      window.apiProyecto &&
      typeof window.apiProyecto.recortarMargenesPDF === "function"
    ) {
      try {
        if (btnMenuRecortar) {
        ...
        if (respuesta.exito) {
          alert(
            "¡Página recortada correctamente y guardada en el PDF original!",
          );
  ...
  // 2. Ejecutar Creación de PDF desde Miniaturas
  async function ejecutarCreacionPdf() {
    ...
    // Mostrar el modal
    if (modalCrearPdf) {
      // Ocultar cualquier error anterior y limpiar clases
      if (modalErrorMensaje) {
        modalErrorMensaje.classList.add("oculto");
      }
      modalCrearPdf.classList.remove("modal-vibrar");

      if (inputNuevoNombre) {
        inputNuevoNombre.value = ''; // Vaciar el input por requerimiento del usuario
      }
      modalCrearPdf.classList.remove("oculto");
      if (inputNuevoNombre) {
  ...
  // Controladores de los botones del modal de creación de PDF
  if (btnModalCancelar) {
    btnModalCancelar.addEventListener("click", () => {
      if (modalCrearPdf) {
        modalCrearPdf.classList.add("oculto");
      }
    });
  }

  if (modalCrearPdf) {
    modalCrearPdf.addEventListener("click", (e) => {
      // Si el clic es en el overlay (fondo) y no en el contenido
      if (e.target === modalCrearPdf) {
        modalCrearPdf.classList.add("oculto");
      }
    });
  }
  ...
  if (btnModalAceptar) {
    btnModalAceptar.addEventListener("click", async () => {
      ...
      if (!/^\d{1,5}$/.test(nuevoNombre)) {
        ...
        if (modalCrearPdf) {
          modalCrearPdf.classList.remove("modal-vibrar");
          void modalCrearPdf.offsetWidth; // Forzar reflujo de renderizado
          modalCrearPdf.classList.add("modal-vibrar");
        }
      ...
      const paginasACortar =
        paginasSeleccionadas.size > 0
          ? Array.from(paginasSeleccionadas).sort((a, b) => a - b)
          : [paginaActiva];

      if (
        window.apiProyecto &&
        typeof window.apiProyecto.cortarPaginasPDF === "function"
      ) {
        try {
          if (btnMenuCortar) {
  ...
  if (inputNuevoNombre) {
    inputNuevoNombre.addEventListener("keydown", (evento) => {
      if (evento.key === "Enter") {
        evento.preventDefault();
        if (btnModalAceptar) {
          btnModalAceptar.click();
        }
      }
    });
  }
  ...
  if (btnMenuCancelarRecorte) {
    btnMenuCancelarRecorte.addEventListener("click", () => {
  ...
  // Cancelar acciones con la tecla Escape (Escape de recorte, escape de creación y cierre de modal)
